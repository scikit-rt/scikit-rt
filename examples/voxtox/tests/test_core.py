...
translations = list(20 * np.random.rand(3) - 10)
rotations = list(10 * np.random.rand(3) - 5)
...
block.add_new(translations_offset, 'DS', [f'{x:.3f}' for x in translations])
...
def test_couch_shifts():
    ...
    for i in [0, 1, 2]:
        assert(translations[i] == pytest.approx(translations2[i], abs=1.e-3))
        assert(rotations[i] == pytest.approx(rotations2[i], abs=1.e-3))
