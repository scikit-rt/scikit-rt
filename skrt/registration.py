...
class Registration:
    ...
    def __init__(self, fixed, moving, pfile, outdir='.', auto_reg=True, 
                 force=False):
        ...
        if isinstance(pfile, str):
            self.pfiles = [pfile]
        self.outdirs = [os.path.join(
            self.outdir, os.path.basename(p).replace('.txt', ''))
        ...
        self.tfiles = [os.path.join(d, 'TransformParameters.0.txt')
                       for d in self.outdirs]
    ...
    def register(self, force=False, apply=False):
        ...
        if not self.registered:
            for i, pfile in enumerate(self.pfiles):
                ...
                self.create_elastix_command(pfile, self.outdirs[i], tfile=tfile,
    ...
    def get_final_image(self, force=False):
        ...
        if not os.path.exists(self.out_path) or force:
            self.out_path = self.transform_image(self.moving_path, 
                                                 self.outdirs[-1], 
                                                 force)
    ...
    def plot_comparison(self, outdir=None, **kwargs):
        ...
        for view in ['x-y', 'y-z', 'x-z']:
            self.comparison.plot_overlay(
                view, 
                save_as=os.path.join(outdir, f'overlay_{view}.pdf'),
                **kwargs
            )
    ...
    def view_init(self, **kwargs):
        ...
        QuickViewer(
            [self.fixed_path, self.moving_path], **kwargs
        )
